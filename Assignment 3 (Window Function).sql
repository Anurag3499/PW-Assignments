-- Assignment - 3 (Window Function)

use mavenmovies;
-- 1 : Rank the customers based on the total amount they've spent on rentals.
select customer_id ,sum(amount) as total_amount_spent ,rank() over( order by sum(amount) desc)  from payment group by customer_id;
...
select  title, film_id ,avg(rental_duration) over(partition by length ) from film where length IS NOT NULL order by film_id;
...
select 
  customer_id, avg(count(rental_id)) over() ,count(rental_id),
...
-- 7 : Identify the customers whose total spending on rentals falls within the top 20% of all customers.
    ...
    SELECT
        customer_id, SUM(amount) AS total_spending,RANK() OVER (ORDER BY SUM(amount) DESC) AS customer_rank
    FROM payment
    GROUP BY customer_id
)
SELECT
